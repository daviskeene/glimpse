...
def _strip_c_style(code: str) -> str:
    ...
    code = _TEXT_BLOCK.sub(lambda m: re.sub(r"[^\n]", '"', m.group(0)), code)
...
def java_strip_package(code: str) -> str:
    """Remove a ``package com.foo;`` declaration.

    Snippets run as a single file: ``javac -d {work}`` would put the class into a package
    subdirectory while the runner invokes the bare type name, so packaged-but-valid LLM
    output would fail with ClassNotFoundException.
    """
    ...
    for index, line in enumerate(stripped.split("\n")):
        ...
        if match:
            # Comment/string blanking preserves offsets up to the match, so the span
            # is valid in the original line too.
            lines[index] = lines[index][: match.start()] + lines[index][match.end() :]
            return "\n".join(lines)
    return code
